...
Il kernel e i suoi test si compilano sempre con `-O2 -g`, anche in Debug: la suite completa gira in circa 38 s (quasi tutto nei test delle booleane, che confrontano con BRepAlgoAPI e con una tassellazione fine di OCCT: quest'ultima da sola prende più di 10 s). Anche l'area di `BRepGProp::SurfaceProperties` sbaglia (fino allo 0.06%) sulle facce estruse da spline tagliate da un piano: lì i test usano come terzo parere l'area e il volume della tassellazione fine, e l'identità V(A∪B) = V(A) + V(B) − V(A∩B).

Roadmap: 0–8 fatti (infrastruttura, curve, superfici, topologia B-rep con controllo di validità, estrusione di profili qualsiasi, proprietà di massa, SP-curve, intersezioni con un piano e booleane, marching tra superfici non piane con le booleane tra cilindri e fianchi estrusi). Seguono: contatti tangenti (compresi i cilindri di raggio uguale con assi incidenti, le cui curve si incrociano), fusione delle facce sulla stessa superficie dopo le booleane (come `ShapeUpgrade_UnifySameDomain`), poi il passaggio dell'app al nuovo kernel dove i test lo confermano; più avanti i semi del marching per sfere, coni, tori e B-spline. Ogni nuovo modulo va con i suoi test contro OCCT.
